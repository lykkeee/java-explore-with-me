...
public interface StatsRepository extends JpaRepository<Stat, Long> {
    @Query(value = "select new ru.practicum.HitResponseDto(s.app, s.uri, count(s.ip) as hits) from Stat as s " +
    ...
    @Query(value = "select new ru.practicum.HitResponseDto(s.app, s.uri, count(distinct s.ip) as hits) from Stat as s " +
            "where s.uri in :uris and s.timestamp between :start and :end " +
